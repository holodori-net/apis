...
Public ranking data is available through `music`, `marathon`, and `profile`.
The `asset` client reports expired content asset IDs, while `health` checks the
serving state without requiring authentication.
...
`api.health`, `api.system`, and `api.accountMigration`. The older top-level
authentication and master methods remain available as deprecated delegations.
Transport implementations are also available from `holodori-apis/transports`;
protobuf and gRPC helpers are available from `holodori-apis/low-level`.
...
path `/rpc.api.<Service>/<Method>`. The SDK implements 42 of those methods:
...
evaluation APIs. Public-information follow-up candidates include creative-chart
discovery and mini-game leaderboards. Account-specific history, gifts,
`AroundSelf` rankings, and read-state operations are intentionally outside the
current public-information scope.
