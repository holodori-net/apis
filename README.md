...
`api.splashBall`, `api.comboCardGame`, `api.health`, `api.system`, and
...
path `/rpc.api.<Service>/<Method>`. The SDK implements 56 of those methods:
...
| ComboCardGame         |           1 |        6 | `GetRankingInfo`                                                                 |
...
evaluation APIs. Public-information follow-up candidates include cooking-puzzle
rankings, event-specific rankings, and read-only catalogue gaps. Account-specific
history, gifts, `AroundSelf` rankings, and read-state operations are
intentionally outside the current public-information scope.
