...
`api.gacha`, `api.system`, and `api.accountMigration`. The older top-level
...
path `/rpc.api.<Service>/<Method>`. The SDK implements 27 of those methods:
...
`Gacha/ListHistory`, the Gift list/count/history methods,
`CharacterHistory/List`, and public profile and ranking reads.
